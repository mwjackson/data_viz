...
float dataMin = MAX_FLOAT; //used to store min and max data values takes a max value and works way through the column to get the min value (from hight to low
float dataMax = MIN_FLOAT; //takes min value and goes up to get high value
...
void setup()
{
  ...
  dataTable = loadTable("co2.csv","header,csv");
  ...
  {
    float currRow = dataTable.getFloat(row,"2010");
    if (Float.isNaN(currRow)) println("row is nan: " + row); 
    dataMin = min(dataMin,dataTable.getFloat(row,"2010"));
    dataMax = max(dataMax, dataTable.getFloat(row,"2010"));
  }
println(dataMin); 
println(dataMax);
}
...
void draw()
{
  ...
  strokeWeight(0.9); //thickness of outline
  ...
  for (int row=0; row<dataTable.getRowCount(); row++)
  {
    ...
    float badTeeth     = dataTable.getFloat(row,"2010");
    ...
    float circleSize = map(badTeeth,dataMin,dataMax,1,50);
